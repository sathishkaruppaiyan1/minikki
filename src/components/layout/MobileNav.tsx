        ...
        {navItems.map((item) => {
          ...
          if (item.action) {
            return (
              <button
                key={item.label}
                onClick={item.action}
                className="flex flex-col items-center gap-1 px-4 py-2 transition-colors text-muted-foreground hover:text-foreground"
              >
                <Icon />
          ...
          return (
            <Link
              key={item.label}
              to={item.href!}
              className={`flex flex-col items-center gap-1 px-4 py-2 transition-colors ${isActive ? "text-primary" : "text-muted-foreground hover:text-foreground"
                }`}
            >
              <Icon />
              <span className="text-xs font-medium">{item.label}</span>
